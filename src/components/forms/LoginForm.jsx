  ...
  return (
    <motion.form initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.4 }} onSubmit={loginHandle} className='flex-1 flex flex-col  gap-4'>
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: .6 }} className='w-full flex flex-col'>
        ...
        <input type="password" id='password' name='password' required value={formData.password} onChange={handleChange} className='w-full px-3 p-1 rounded-lg outline-none border border-sky-400/10' />
      </motion.div>
      ...
      <motion.button initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.7 }} type='submit' className='bg-sky-600/80 hover:bg-sky-600 text-white rounded-lg cursor-pointer '>Next</motion.button>
    </motion.form>
  )
}
