...
const EditBrandForm = ({ brandToEdit, onClose }) => {
    const [formData, setFormData] = useState({
        name: brandToEdit?.name || '',
        description: brandToEdit?.description || ''
    })
    const { fetchBrand } = useContext(Context)
    ...
    const updateBrand = async (e) => {
        e.preventDefault()
        if (!brandToEdit?.brand_id) return
        setIsSubmitting(true)
        try {
            const response = await axios.put('/api/brand', {
                id: brandToEdit.brand_id,
                ...formData
    ...
    return (
        <form onSubmit={updateBrand} className='flex flex-col w-full bg-white'>
            {/* Header */}
            <div className='flex items-center justify-between p-6 border-b border-slate-100'>
                <h2 className='text-lg font-bold text-slate-800 tracking-tight'>Edit Brand</h2>
                <button 
                    type='button' 
                    onClick={onClose} 
            ...
            <div className='p-6 flex flex-col gap-5'>
                <div className='flex flex-col gap-1.5'>
                    <label htmlFor="edit_brand_name" className='text-sm font-semibold text-slate-700'>Brand Name <span className='text-rose-500'>*</span></label>
                    <input 
                        type="text" 
                        id='edit_brand_name' 
                        name='name' 
                ...
                <div className='flex flex-col gap-1.5'>
                    <label htmlFor="edit_brand_description" className='text-sm font-semibold text-slate-700'>Description</label>
                    <textarea 
