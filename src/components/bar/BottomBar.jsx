...
const BottomBar = () => {
  ...
  return (
    <div  className='w-full flex sm:hidden flex-row items-center justify-between fixed bottom-0 right-0 h-14 px-4 bg-white z-50 text-2xl'>
          ...
          <Link href={'/cart'} className={`flex flex-col items-center justify-center relative `}> {cart.items.length>0 && <span className='w-5 rounded-full h-5 absolute -top-4 bg-sky-600 text-[14px] text-center flex items-center justify-center text-white'>{cart.items.length}</span>}<CiShoppingCart /><span className='text-[8px]'>Cart</span></Link>
    </div>
  )
}
